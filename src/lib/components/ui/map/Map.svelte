<script lang="ts">
	...
	interface Props {
		...
		/**
		 * Fit the map to these bounds [sw, ne] as [[minLng, minLat], [maxLng, maxLat]].
		 * When bounds change, the map animates to fit them.
		 */
		bounds?: [[number, number], [number, number]];
		/** Padding in pixels for fitBounds. Can be asymmetric to offset for UI panels. */
		boundsPadding?: number | MapLibreGL.PaddingOptions;
		/**
	...
	$effect(() => {
		...
		// Skip if controlled mode is enabled or bounds are provided
		if (bounds) return;
